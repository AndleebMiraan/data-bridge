...
def sensorAdd(request):
    ...
    exist_data = models.Sensor.objects.filter(
        name=request.POST['name'], sensor_type_id=request.POST['sensor_type_id']).filter(deleted=0)
    if len(exist_data) > 0:
        context.update({
            'status': 539,
...
def sensorEdit(request):
    ...
    if not request.POST['name'] or not request.POST['sensor_type_id'] :
        ...
        return JsonResponse(context)
    exist_data = models.Sensor.objects.filter(
        name=request.POST['name'], sensor_type_id=request.POST['sensor_type_id']).exclude(pk=request.POST['id']).filter(deleted=0)
    if len(exist_data) > 0:
        context.update({
            'status': 542,
            'message': "Sensor with this name and sensor type already exists.",
