...
def importExcel(request):
    if request.method == "POST":
        ...
        if 'section_sensor_mapping_required' in request.POST.keys():
            # f = open("templates/constants/section_sensor_mapping_columns.txt", "w+")
            # f.write("")
            # f.close()
                            ...
                            with transaction.atomic():
                                ...
                                for column in df.columns:
                                    ...
                                    if all(v == 0 for v in df[column].tolist()):
                                        ...
                                        sensor_name = column
                                    if not all(v == 0 for v in df[column].tolist()) and sensor_name and "emperature" not in column:
                                ...
                                for value in df.iloc[:, 0]:
                                    date_time = None
                                    # converting string datetime value to python datetime object
                                    if len(value.split(":"))==3:
                                        if "." in value.split(":")[2]:
                                            date_time = datetime.strptime(value, date_time_format_with_fractional_second)
                                        else:
                                            date_time = datetime.strptime(value, date_time_format_with_second)
                                    elif len(value.split(":"))==2:
                                        date_time = datetime.strptime(value, date_time_format_without_second)
                                ...
                                for i, row in df.iterrows():
                                    time_stamp_str = row.iloc[0]
                                    date_time = None
                                    # converting string datetime value to python datetime object
                                    if len(time_stamp_str.split(":")) == 3:
                                        if "." in time_stamp_str.split(":")[2]:
                                            date_time = datetime.strptime(time_stamp_str, date_time_format_with_fractional_second)
                                        else:
                                            date_time = datetime.strptime(time_stamp_str, date_time_format_with_second)
                                    elif len(time_stamp_str.split(":")) == 2:
                                        date_time = datetime.strptime(time_stamp_str, date_time_format_without_second)
                                    ...
                                    for section_str, sensors in section_sensor_dict.items():
                                        ...
                                        for sensor_str in sensors:
                                            ...
                                            if api_models.Sensor_Data.objects.filter(section_sensor_mapping=section_sensor_mapping,time_stamp=time_stamp_obj,data=data).exists():
                                                sensor_data=api_models.Sensor_Data.objects.filter(
                                                    section_sensor_mapping=section_sensor_mapping,
                                                    time_stamp=time_stamp_obj,
                                                    data=data).first()
                                                sensor_data.section_sensor_mapping=section_sensor_mapping
                                                sensor_data.time_stamp = time_stamp_obj
                                ...
                                api_models.Sensor_Data.objects.bulk_create(sensor_datas)


                                # reader = csv.reader(csvfile)
                                # first_row = next(reader, None)
                                # # Removing Date Time Column Name
                                # del first_row[0]
...
def getChartData(request):
    if request.method == "POST":
        ...
        if mapping is None:
            ...
            # Appending Strain Data
            section_sensor_mappings_in_the_sensor_data=api_models.Section_Sensor_Mapping.objects.filter(
                section=section,sensor__sensor_type__name="Strain"
                ).filter(
                sensor_data__isnull=False
                )
            strain_sensors_in_sensor_data=api_models.Sensor.objects.filter(
                section_sensor_mapping__in=section_sensor_mappings_in_the_sensor_data
            ).distinct()

            for sensor in strain_sensors_in_sensor_data:
                data=[]
                section_sensor_mapping=api_models.Section_Sensor_Mapping.objects.filter(section=section,sensor=sensor).first()
                sensor_datas=api_models.Sensor_Data.objects.filter(section_sensor_mapping=section_sensor_mapping)
                for sensor_data in sensor_datas:
                    data.append(
                        [
                            int(createMilisecondsByDate(str(sensor_data.time_stamp.date_time).replace(" ", "T"))),
                            float(sensor_data.data)
                        ]
                    )
                series["Strain"].append(
                    {
            ...
            # Appending Tilt Data
            section_sensor_mappings_in_the_sensor_data=api_models.Section_Sensor_Mapping.objects.filter(
                section=section,sensor__sensor_type__name="Tilt"
                ).filter(
                sensor_data__isnull=False
                )
            tilt_sensors_in_sensor_data=api_models.Sensor.objects.filter(
                section_sensor_mapping__in=section_sensor_mappings_in_the_sensor_data
            ).distinct()

            for sensor in tilt_sensors_in_sensor_data:
                data=[]
                section_sensor_mapping=api_models.Section_Sensor_Mapping.objects.filter(section=section,sensor=sensor).first()
                sensor_datas=api_models.Sensor_Data.objects.filter(section_sensor_mapping=section_sensor_mapping)
                for sensor_data in sensor_datas:
            ...
            # Appending Settlement(P) Data
            section_sensor_mappings_in_the_sensor_data=api_models.Section_Sensor_Mapping.objects.filter(
                section=section,sensor__sensor_type__name="Settlement(P)"
                ).filter(
                sensor_data__isnull=False
                )
            settlement_p_sensors_in_sensor_data=api_models.Sensor.objects.filter(
                section_sensor_mapping__in=section_sensor_mappings_in_the_sensor_data
            ).distinct()

            for sensor in settlement_p_sensors_in_sensor_data:
                data=[]
                section_sensor_mapping=api_models.Section_Sensor_Mapping.objects.filter(section=section,sensor=sensor).first()
                sensor_datas=api_models.Sensor_Data.objects.filter(section_sensor_mapping=section_sensor_mapping)
                for sensor_data in sensor_datas:
            ...
            # Appending Settlement(F) Data
            section_sensor_mappings_in_the_sensor_data=api_models.Section_Sensor_Mapping.objects.filter(
                section=section,sensor__sensor_type__name="Settlement(F)"
                ).filter(
                sensor_data__isnull=False
                )
            settlement_f_sensors_in_sensor_data=api_models.Sensor.objects.filter(
                section_sensor_mapping__in=section_sensor_mappings_in_the_sensor_data
            ).distinct()

            for sensor in settlement_f_sensors_in_sensor_data:
                data=[]
                section_sensor_mapping=api_models.Section_Sensor_Mapping.objects.filter(section=section,sensor=sensor).first()
                sensor_datas=api_models.Sensor_Data.objects.filter(section_sensor_mapping=section_sensor_mapping)
                for sensor_data in sensor_datas:
